...
| カテゴリ | 数量 | 概要 |
|----------|------|------|
| エージェント | 65 | ドメイン特化型 AI エージェント |
| 共通プロトコル | 27+ | エージェント動作を統一するルール群 |
| カスタムコマンド | 6 | セッション内ワークフローモード |
| スキル | 4 | 再利用可能な手順スキル |
| Hook | 3 | PreToolUse / PostToolUse / Stop |
...
| 特性             | 実装                                       |
+------------------+--------------------------------------------+
| Security-First   | 3-Hook体制 + Sandbox + Permissions         |
...
```
     ...
     (全65)          (nexus,builder, (nexus,analyst,
                      radar)          pulse)
```
...
│  │ tool-risk.js │→│ post-tool-   │  │ stop-     │  │
│  │              │  │ use.js       │  │ hook.js   │  │
│  ├──────────────┤  ├──────────────┤  ├───────────┤  │
│  │ リスク分類   │  │ ツールログ   │  │ セッション│  │
│  │ Safety Gate  │  │ 記録         │  │ サマリ生成│  │
│  │ ブロック/確認│  │ .context/    │  │ .context/ │  │
│  │ コンテキスト│  │  tool-log    │  │  sessions │  │
│  │  注入       │  │  .jsonl      │  │  /*.jsonl │  │
│  └──────────────┘  └──────────────┘  └───────────┘  │
└─────────────────────────────────────────────────────┘
```
...
| 3-Hook体制 | ツール実行のライフサイクル全体をカバー | Hook 実行による微小なレイテンシ |
